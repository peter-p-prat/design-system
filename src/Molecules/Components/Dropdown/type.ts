...
import {SearchInputProps} from "../../../Atoms/Components/InputFields";
...
export const isMenuDropdownOption = (
  option: DropdownOption
): option is DropdownMenuOption => "onClick" in option;
...
export type DropdownCollapsibleValue = Record<
  string,
  DropdownCollapsibleGroupValue
>;
export type DropdownCollapsibleOptions = Record<
  string,
...
export type DropdownCollapsibleTrigger =
  | React.ReactNode
  | DropdownCollapsibleTriggerConfigs;

export const isCustomTrigger = (
  trigger: DropdownCollapsibleTrigger
): trigger is React.ReactNode => isValidElement(trigger);

export interface DropdownCollapsibleProps
  extends Partial<
    Omit<HTMLAttributes<HTMLDivElement>, "defaultValue" | "onChange">
  > {
