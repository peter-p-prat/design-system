...
import {SearchInput, SearchInputProps} from "../InputFields";

import useModalMode from "./hooks/useModalMode";
import {DropdownIconComponent, DropdownLabel, DropdownList, DropdownOptionComponent} from "./components";
import {DropdownFormTriggerSizes, DropdownModes, DropdownOption, DropdownProps} from "./type";
...
export const Dropdown = ({
    ...
    const [internalValue, setInternalValue] = useState<DropdownProps["value"]>(value || defaultValue);
    const [selectedValue, setSelectedValue] = useState<DropdownProps["value"]>(value || defaultValue);
    ...
    const labelValues: DropdownOption | undefined = useMemo(() => {
        ...
        return {
            label: label ?? (placeholder ? <span className={styles.placeholder}>{placeholder}</span> : undefined),
            value: labelValue?.value ?? "",
            icon: labelIcon,
        };
    }, [multiselect, mode, labelValue, label, labelIcon, placeholder, itsIconAndTrigger, itsOnlyTriggerIcon]);
    ...
    const isOptionSelected = (option: DropdownOption) => {
        const selectedValuesSet = new Set(internalValue?.map((mappedOption) => mappedOption.value));
    ...
    const onClickOption = (clickedOption: DropdownOption) => {
        if (multiselect) {
            setInternalValue((prevValue) => {
                const prevValueArray = !!prevValue && Array.isArray(prevValue) ? prevValue : [];
                const optionIsSelected = isOptionSelected(clickedOption);
                const newSelectedValue = optionIsSelected
                    ? prevValueArray.filter((selectedOption) => selectedOption.value !== clickedOption.value)
        ...
        if (!submitActionButton && mode !== DropdownModes.MENU) setSelectedValue([clickedOption]);
    ...
    const filterOptions = useCallback(
        (optionValue: DropdownOption, optionIndex: number, optionsArray: DropdownOption[]): boolean => {
            if (!enableSearchInput || !searchTextSubmit) return true;
            if (onSearchFilter) return onSearchFilter(searchTextSubmit, optionValue, optionIndex, optionsArray);
    ...
    const handleEnterKeyOnSearchInput = () => {
        ...
        requestAnimationFrame(() => {
            const dropdownOptionElement = document.activeElement as HTMLDivElement | null;
            ...
            >
                ...
                <DropdownList
                    ...
                    searchInput={
                        enableSearchInput && (
                            <div
                                className={className(styles.searchInputContainer, conditionalClass([styles.desktopMode, !!desktopMode]))}
                                ...
                                tabIndex={0}
                            >
                                <SearchInput {...defaultSearchInputProps} {...searchInputProps} />
                            </div>
                        )
                    }
                    options={options
                        .filter((option, index, array) => filterOptions(option, index, array))
                        .map((option, i) => (
