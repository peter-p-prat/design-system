...
import {Dropdown, DropdownAlignments, DropdownFormTriggerSizes, DropdownModes, DropdownOption, DropdownProps} from "@app/Atoms";
import {icons, IconsNames, TenantLogo, TenantLogoColor, TenantLogoSize, TenantLogoVariable, TenantSymbolPosition} from "@app/Foundations";
import {tenantLogosConfig} from "@app/Foundations/Components/TenantLogo/stories/tenantLogosConfig";
...
const meta = {
    ...
    argTypes: {
        ...
        onChange: {
            ...
            description: "Triggers when the value on the Dropdown changes. Passes an DropdownOption in the callback",
        },
        value: {
            control: {type: "object"},
            table: {
        ...
        customLabelClassName: {
            ...
            description: "Custom classname to be forwarded to the dropdown label element. Can be used to give special styles to the label",
        },
        placeholder: {
            control: {type: "text"},
            table: {
        ...
        disableCloseAfterSelection: {
            ...
            description: "Controls whether to disable the default feature of closing the dropdown when selecting an option",
        },
        ...
        triggerIcon: {
            ...
            description: "The icon to replace the default chevron down on the trigger",
        },
        light: {
            control: {type: "boolean"},
            table: {
        ...
        useRadioButtons: {
            ...
            description: "Enabling this make the options show radio buttons at the beginning",
        },
        open: {
            control: {type: "boolean"},
            table: {
...
export const Default: Story = {
    args: {
        ...
        getOptionDisabled: (option) => ["optionDisabled", "optionDisabledSelected"].includes(option?.value ?? ""),
        getOptionSelected: (option, dropdownValue) =>
            dropdownValue?.some((selectedOption) => selectedOption.value === option?.value) ||
            ["optionSelected", "optionDisabledSelected"].includes(option?.value ?? ""),
    },
    decorators: [
        (Story) => {
            const {props} = Story();
            return (
                <DropdownWrapper withBackground={props.disableTransparency} light={props.light}>
                    <Story />
                </DropdownWrapper>
            );
        },
...
export const LongText: Story = {
    args: {
        ...
        options: [
            ...DEFAULT_OPTIONS,
            {
                label: "Option With Super long text Option With Super long text Option With Super long text",
                value: "optionLong",
...
export const UseAsControlledComponent: Story = {
    render: (args) => {
        const [selectedOption, setSelectedOption] = useState<DropdownOption>(args.options[1] as DropdownOption);
        ...
        return (
            <DropdownWrapper withBackground={args.disableTransparency} light={args.light}>
                <Dropdown {...args} value={[selectedOption]} onChange={handleChange} />
                <p>Here we are using and internal variable to change and update the value</p>
    ...
    render: (args) => {
        ...
        return (
            <DropdownWrapper withBackground={args.disableTransparency} light={args.light}>
                ...
                />
                {selectedOptions?.map((option) => <p key={option.value}>Action value: {option.label}</p>)}
            </DropdownWrapper>
        );
    },
    args: {
    ...
    args: {
        ...
        options: [
            ...
            },
            {
                label: "MyUs",
                value: "myus",
                icon: {url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdsa33myPmeFAeZ_iX0OCUZvVWy7uZlrm5Jw&s", alt: "MyUs"},
            },
            {
        ...
        options: [
            {
                ...
                icon: (
                    <TenantLogo
                        variable={TenantLogoVariable.SYMBOL}
                        color={TenantLogoColor.LIGHT_BG}
                        tenantLogos={tenantLogosConfig}
                        position={TenantSymbolPosition.CENTER}
                        size={TenantLogoSize.XS}
                        alt="LATAM Pass"
                    />
                ),
            },
